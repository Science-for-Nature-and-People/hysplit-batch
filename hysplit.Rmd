...
system(sprintf("ln -s %s %s", arl_year_path, file.path(run_dir, run_year)))

# List all the EMITFILES
emmitimes_files <- list.files(path = shared_dir, pattern = "Scenario")
emmitimes_files


foreach(scenarios_file = emmitimes_files) %dopar% {
  
  model_run <- gsub("^[^_]*_", replacement = "", scenarios_file) 
  model_run <- gsub("\\.[^.]*$", replacement = "", model_run) 
